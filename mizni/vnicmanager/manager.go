...
type devNetConfGetter interface {
	GetDevNetConf(name, nsPath string) (*net.IPNet, *net.IP, string, error)
}

type nsMigrator interface {
	Migrate(nameFrom, nsFrom, nameTo, nsTo string, ipnet *net.IPNet, gw *net.IP) error
}
...
func (m Manager) Plug(vn *vnic.VNIC) (*vnic.EPnic, error) {
	...
	ipNet, gw, mac, err := m.ConfGetter.GetDevNetConf(dev, alcorNSPath)
	if err != nil {
		return nil, fmt.Errorf("Plug vnic %q failed, unable to get settings: %v", vn.PortID, err)
	}

	if err := m.NSMigrator.Migrate(dev, alcorNSPath, vn.Name, m.NScni, ipNet, gw); err != nil {
